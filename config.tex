...
\newcommand{\yourBirthdate}{2000年1月}
\newcommand{\yourContact}{\yourPhone} % 默认联系方式与手机号相同，可以自行修改
\newcommand{\yourExpectedGraduationTime}{2026年1月}
...
\def\needAvatar{false} % 是否需要头像，true/false
\def\AvatarWidth{0.13} % 头像宽度，推荐0.12~0.15，与个人信息模块宽度之和小于0.95
\def\PersonalInformationWidth{0.80} % 个人信息模块宽度
...
% 竞赛经历
\def\needCompetitions{true} % 是否需要竞赛经历模块，true/false
...
% 页眉与校徽与主题色
\def\useDefaultTheme{true} % 是否使用默认主题，详见 README.md；建议：true
\def\useSchoolLogo{true} % 是否使用学校logo，如设为true，需要替换 images/school_logo.png 为你的校徽+校名
\def\useSchoolName{false} % 是否使用学校名称，true/false；仅当\useSchoolLogo为false时生效
\def\schoolLogo{images/school_logo.png} % 校徽+校名图片路径
\def\schoolLogoWidth{0.15} % 校徽+校名占行宽的比例（根据logo宽度调整，4字学校大约0.15，6字学校大约0.2，）
...
% 水印设置
\def\needWatermark{false} % 是否需要水印，true/false
...
% 字体设置
% 参见 README.md -> 自定义字体
\def\useDefaultFont{false} % 是否使用系统默认字体，详见 README.md
\def\fontPath{fonts/} % 字体文件路径
\def\customFontFamilyCH{SweiSpring} % 仅当\useDefaultFont被设置为false时生效；需与字体文件名对应
\def\fontFileTypeCH{.ttf} % 字体文件类型，.ttf/.otf（此时字体文件应为：fonts/SweiSpring.ttf）
\def\fontBoldFontCH{* Bold} % 粗体字体文件名（此时粗体字体文件应为：fonts/SweiSpring Bold.ttf）
\def\customFontFamilyEN{\customFontFamilyCH} % 建议英文与中文用统一字体
